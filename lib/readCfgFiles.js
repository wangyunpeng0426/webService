...
const { WebsocketClients } = require('./websocketClients')  //websocket连接各个进程
const { MonitorModular }  = require('./monitor')            //监控模块
...
function webServiceAnalyticInfor () {
    ...
    this.oSystemMonitorModular      = null               //系统状态监控模块对象
    ...
    this.AnalyticLocalCfgJson = (strJson) => {
        ...
        if(1 == Json.nodecfg.systemMonitorModular){
            _this.oSystemMonitorModular = new MonitorModular()
            _this.oSystemMonitorModular.Init(Json.nodecfg.monitorReportWay,Json.nodecfg.monitorReportIp,Json.nodecfg.monitorReportPort,Json.nodecfg.monitorReportMethod)
        }
    }
    /**
    ...
    this.AnalyticNodeInforCfgJson = (strJson) => {
        ...
        if(!webSocketClients){
            ...
            webSocketClients.Init(_this.oWebsocketService,_this.oSystemMonitorModular)
        }
        if(Json.nodecfg.vics){
    ...
    this.LocalCfgDefaultContent = () => {
        ...
        <nodecfg>
            ...
            <monitorReportWay>rest</monitorReportWay>
            <monitorReportIp>192.168.5.186</monitorReportIp>
            <monitorReportPort>5010</monitorReportPort>
            <monitorReportMethod>MonitorApis/SendDatas</monitorReportMethod>
        </nodecfg>`
        return xml;
    }
    /**
